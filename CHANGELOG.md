...
- 《不可能的任务，把 Agent 逼得像人了》中“这份整理”的链接从 Slax Reader 快照改为 wlj.me 原文，并同步修正 zh-tw / en / ja / ko 版本。
...
- 9 月 8 日到 10 日的每日视频刷新连续三次失败：频道扫描把三条火山停航新闻当成 AI 视频，模型拒绝按格式总结，`emit.ts` 直接抛错，同批的五条真实视频跟着一起卡住。
- `emit.ts`：模型两次都没给出 JSON 时跳过这一条候选并打 WARN，其余候选照常出 PR；其它错误（网络、鉴权）仍然抛出，留给次日重试。整批都被跳过时按“无新增”退出。
- `01_scan_channels.py`：强信号词 `national ai` 加词边界，此前会命中 “inter**national ai**rport”。
- `fetch-transcripts.ts`：字幕语言顺序在 `en` 之后加 `en-orig`（多音轨视频的英文原轨）。v095 首次抓取时 `en` 被 YouTube 限流（429），退到机翻的 zh-Hans 轨，只剩一段乱文且没有英文，转写覆盖检查失败。
