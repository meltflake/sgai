...
Take: SUTD's existence saves the Singapore university system from **"the risk of NUS-and-NTU homogeneous competition"** — it takes the crossover-innovation route and complements the two comprehensive universities.`,
      ...
      },
      {
        ...
        whatItIs: `AISG 支持、南洋理工大学张捷教授领导、联合美国麻省理工学院（含 Jacob Andreas 副教授）的研究合作项目，主题是可信、可解释的人机协作组合优化系统。核心思路是把自然语言变成人与 AI 之间描述优化问题的鲁棒接口：用户用日常语言提出需求，系统自动将其转成规范的组合优化问题，求解后再以人能理解的方式解释结果。`,
        whatItIsEn: `An AISG-supported research collaboration project led by Professor Zhang Jie of Nanyang Technological University and jointly conducted with the Massachusetts Institute of Technology in the United States (including Associate Professor Jacob Andreas), focusing on trustworthy and interpretable human-AI collaborative combinatorial optimization systems. The core approach is to transform natural language into a robust interface for describing optimization problems between humans and AI: users present requirements in everyday language, the system automatically converts them into standardized combinatorial optimization problems, solves them, and then explains the results in ways humans can understand.`,
        whatItIsJa: `AISG の支援を受け、南洋理工大学の張傑教授が主導し、米国マサチューセッツ工科大学（Jacob Andreas 副教授を含む）と共同で進める研究協力プロジェクトです。テーマは信頼性と解釈可能性を備えた人間-AI 協働型の組み合わせ最適化システムです。核心的なアイデアは、自然言語を人間と AI 間で最適化問題を記述するためのロバストなインターフェースに変換することです。ユーザーが日常言語で要件を提示すると、システムは自動的にそれを標準的な組み合わせ最適化問題に変換し、求解した後、人間が理解できる方法で結果を説明します。`,
        whatItIsKo: `AISG의 지원 하에 남양이공대학교(NTU)의 장제 교수가 주도하고, 미국 매사추세츠공과대학교(MIT)의 Jacob Andreas 부교수를 포함한 연구팀과 협력하는 연구 프로젝트로서, 신뢰할 수 있고 설명 가능한 인간-AI 협력 조합 최적화 시스템을 다룹니다. 핵심 아이디어는 자연어를 인간과 AI 간의 최적화 문제 기술을 위한 견고한 인터페이스로 변환하는 것입니다. 사용자가 일상 언어로 요구사항을 제시하면, 시스템이 이를 자동으로 표준 조합 최적화 문제로 변환한 후 풀이하고, 인간이 이해할 수 있는 방식으로 결과를 설명합니다.`,
        aiRelevance: `项目落在「可信 AI」与「组合优化」的交叉点。传统优化要求把问题写成精确的数学形式，门槛高；该项目让大模型承担「从模糊的人类语言中提炼优化问题」的角色，再进行鲁棒求解并给出可解释反馈，降低非专家使用优化技术的门槛。它同时是可解释 AI 的基础研究——不只给出答案，还要让人理解原因。`,
        aiRelevanceEn: `The project sits at the intersection of "trustworthy AI" and "combinatorial optimization." Traditional optimization requires problems to be expressed in precise mathematical form, which has a high barrier to entry; this project assigns large language models the role of "extracting optimization problems from ambiguous human language," then conducts robust solving and provides interpretable feedback, lowering the threshold for non-experts to use optimization techniques. It is simultaneously foundational research in interpretable AI—not just providing answers, but enabling people to understand the reasoning.`,
        aiRelevanceJa: `このプロジェクトは「信頼性のある AI」と「組み合わせ最適化」の交点に位置しています。従来の最適化は、問題を正確な数学形式で記述することが必要で、敷居が高いです。本プロジェクトは大規模言語モデルに「曖昧な人間の言語から最適化問題を抽出する」役割を担わせ、その後、ロバストに求解し、解釈可能なフィードバックを提供することで、非専門家が最適化技術を使用する敷居を低くしています。同時に、解釈可能な AI の基礎研究でもあります。答えを与えるだけでなく、人々が理由を理解できるようにする必要があります。`,
        aiRelevanceKo: `이 프로젝트는 「신뢰할 수 있는 AI」와 「조합 최적화」의 교점에 자리합니다. 전통적인 최적화는 문제를 정확한 수학 형식으로 표현하도록 요구하므로 진입 장벽이 높습니다. 본 프로젝트는 대규모 언어 모델이 「모호한 인간 언어로부터 최적화 문제를 추출」하는 역할을 하도록 하고, 이후 견고한 풀이를 수행하며 설명 가능한 피드백을 제공하여 비전문가가 최적화 기술을 사용하는 진입 장벽을 낮춥니다. 이는 동시에 설명 가능한 AI의 기초 연구이기도 합니다. 단순히 답을 제시하는 것이 아니라 사람들이 그 이유를 이해하도록 해야 합니다.`,
        singaporeRelevance: `项目由 NTU 主导、被纳入 AISG 的研究合作网络，是新加坡「用基础研究支撑产业落地」路线的一个样本。目标应用领域——物流、制造、金融——都是新加坡的支柱行业，对可解释、可信的决策工具有实际需求。与 MIT 的合作也体现 AISG 把新加坡研究接入全球顶尖网络的定位。`,
        singaporeRelevanceEn: `Led by NTU and incorporated into AISG's research collaboration network, the project exemplifies Singapore's approach of "using fundamental research to support industry application." Target application domains—logistics, manufacturing, and finance—are all pillar industries for Singapore with genuine demand for interpretable and trustworthy decision-making tools. The collaboration with MIT also reflects AISG's positioning to integrate Singapore's research into global top-tier networks.`,
        singaporeRelevanceJa: `このプロジェクトは NTU が主導し、AISG の研究協力ネットワークに組み込まれており、シンガポール「基礎研究で産業応用を支える」というアプローチの一つの事例です。目標とする応用領域——物流、製造、金融——はいずれもシンガポールの基幹産業であり、解釈可能で信頼性のある意思決定ツールに対する実際の需要があります。MIT との協力は、AISG がシンガポール研究をグローバルなトップレベルのネットワークに接続するという役割を体現しています。`,
        singaporeRelevanceKo: `이 프로젝트는 NTU가 주도하며 AISG의 연구 협력 네트워크에 포함되어 있으며, 싱가포르의 「기초 연구로 산업 응용을 지원」하는 경로의 사례입니다. 목표 응용 분야인 물류, 제조, 금융은 모두 싱가포르의 주요 산업이며, 설명 가능하고 신뢰할 수 있는 의사 결정 도구에 대한 실제 수요가 있습니다. MIT와의 협력은 또한 AISG가 싱가포르의 연구를 전 세계 최고 수준의 네트워크에 연결시키는 위치를 반영합니다.`,
        entityType: 'program',
        status: 'active',
        sources: [
          {
      ...
      },
      {
        ...
        whatItIs: `PINNACLE 是一种改进物理信息神经网络（PINN）训练效率的自适应采样方法。PINN 用神经网络求解受物理方程约束的问题，训练效果高度依赖「在哪些点上采样」。PINNACLE 基于经验神经切线核（NTK）理论，自动且联合地优化实验点与配置点的选择，不再依赖人工经验布点。`,
        whatItIsEn: `PINNACLE is an adaptive sampling method for improving the training efficiency of Physics-Informed Neural Networks (PINNs). PINNs use neural networks to solve problems constrained by physical equations, and training effectiveness highly depends on "which points to sample from." PINNACLE, based on Empirical Neural Tangent Kernel (NTK) theory, automatically and jointly optimizes the selection of experimental points and collocation points, eliminating reliance on manual experience-based placement.`,
        whatItIsJa: `PINNACLE は、物理情報ニューラルネットワーク（PINN）の訓練効率を改善する自適応サンプリング方法です。PINN はニューラルネットワークを使用して物理方程式の制約を受ける問題を解き、訓練効果は「どの点でサンプリングするか」に大きく依存します。PINNACLE は経験的ニューラル接線核（NTK）理論に基づき、実験点と配置点の選択を自動的かつ統合的に最適化し、人工的な経験に基づく配置に依存しなくなります。`,
        whatItIsKo: `PINNACLE은 물리 정보 신경망(PINN) 훈련 효율을 개선하는 적응형 샘플링 방법입니다. PINN은 신경망을 사용하여 물리 방정식 제약을 받는 문제를 풀며, 훈련 효과는 「어느 지점에서 샘플링하는가」에 크게 의존합니다. PINNACLE은 경험적 신경 탄젠트 커널(NTK) 이론을 기반으로 하여, 실험 지점과 구성 지점의 선택을 자동으로 그리고 합동으로 최적화하므로 더 이상 수동 경험적 배치에 의존하지 않습니다.`,
        aiRelevance: `这是 AI for Science 方向的方法研究。PINN 把深度学习引入科学与工程计算，但采样点的选择长期靠试错，是训练不稳定的主要来源。PINNACLE 把点选择变成可自动优化的环节，在前向问题、反演问题、迁移学习等多类任务上均超过现有基准，提升了 PINN 的通用性与可靠性。`,
        aiRelevanceEn: `This is methodological research in the AI for Science direction. PINN introduced deep learning to scientific and engineering computation, but the selection of sampling points has long relied on trial and error, a primary source of training instability. PINNACLE transforms point selection into an automatically optimizable step, outperforming existing benchmarks on multiple types of tasks including forward problems, inverse problems, and transfer learning, enhancing PINN's generality and reliability.`,
        aiRelevanceJa: `これは AI for Science 方向での方法論研究です。PINN は深層学習を科学および工学計算にもたらしましたが、サンプリング点の選択は長年にわたり試行錯誤に頼っており、訓練の不安定性の主な原因です。PINNACLE はポイント選択を自動的に最適化できるステップに変え、順問題、逆問題、転移学習など複数のタイプのタスクで既存のベンチマークを上回り、PINN の汎用性と信頼性を向上させています。`,
        aiRelevanceKo: `이는 AI for Science 방향의 방법 연구입니다. PINN은 심층 학습을 과학 및 공학 계산에 도입하지만, 샘플링 지점의 선택은 오랫동안 시행착오에 의존해 왔으며, 이는 훈련 불안정성의 주요 원인입니다. PINNACLE은 지점 선택을 자동으로 최적화할 수 있는 단계로 변환하고, 정방향 문제, 역문제, 전이 학습 등 다양한 작업에서 기존 기준을 초과하는 성과를 내며, PINN의 일반성과 신뢰성을 향상시킵니다.`,
        singaporeRelevance: `作为 AISG 的研究产出，PINNACLE 属于新加坡在 AI for Science 基础方法上的积累。科学计算的 AI 化对材料、工程、气候等领域的仿真都有价值，与新加坡将 AI 用于科研与工业仿真的方向一致。`,
        singaporeRelevanceEn: `As a research output of AISG, PINNACLE represents Singapore's accumulation in foundational methods for AI for Science. AI-enabled scientific computing has value for simulation across materials, engineering, and climate domains, aligning with Singapore's direction of applying AI to scientific research and industrial simulation.`,
        singaporeRelevanceJa: `AISG の研究成果として、PINNACLE はシンガポールの AI for Science 基礎方法論における蓄積に属しています。科学計算の AI 化は、材料、工学、気候などの分野のシミュレーションに価値があり、シンガポールが AI を科学研究と工業シミュレーションに活用する方向と一致しています。`,
        singaporeRelevanceKo: `AISG의 연구 산출물로서, PINNACLE은 싱가포르가 AI for Science 기초 방법에서 축적한 성과입니다. 과학 계산의 AI화는 재료, 공학, 기후 등 분야의 시뮬레이션에 가치가 있으며, 싱가포르가 AI를 과학 연구 및 산업 시뮬레이션에 활용하는 방향과 일치합니다.`,
        entityType: 'program',
        status: 'active',
        sources: [
          {
...
A take: **Singapore being able to sit at the AI Safety Summit table is a manifestation of its "national brand equity"** — a mature governance framework, rule-of-law stability, and AI neutrality — letting both the US/UK and China accept it as a counterpart. In an era of AI geopoliticisation, this kind of "neutral, trusted" position is becoming more and more valuable.`,
      ...
      },
      {
        ...
        whatItIs: `AI Singapore 联合 Google 与 VISTEC 举办的第三届东南亚语言峰会，首次移师新加坡境外（曼谷），把区域内从事本地语言 AI 的研究者聚到一起。峰会发布了 SEA-LION v2、Project SEALD、SEACrowd 等成果，并预告面向社区数据的 Project Aquarium 平台，以填补区域高质量数据的空白。`,
        whatItIsEn: `The third Southeast Asia Language Summit, jointly hosted by AI Singapore, Google, and VISTEC, was held for the first time outside Singapore (in Bangkok), bringing together researchers in the region working on local language AI. The summit released results including SEA-LION v2, Project SEALD, SEACrowd, and previewed the Project Aquarium platform for community data to fill the gap in high-quality regional data.`,
        whatItIsJa: `AI Singapore が Google および VISTEC と共同で開催する第 3 回東南アジア言語サミットは、初めて会場をシンガポール外（バンコク）に移し、この地域で地域言語 AI に従事する研究者を一堂に集めました。サミットでは SEA-LION v2、Project SEALD、SEACrowd などの成果が発表され、地域の高品質データの空白を埋めるため、コミュニティデータを対象とした Project Aquarium プラットフォームが予告されました。`,
        whatItIsKo: `AI Singapore가 Google 및 VISTEC과 함께 개최한 제3회 동남아 언어 정상 회담은 처음으로 싱가포르 외부(방콕)로 개최지를 옮겼으며, 지역 내에서 현지 언어 AI에 종사하는 연구자들을 한자리에 모았습니다. 정상 회담은 SEA-LION v2, Project SEALD, SEACrowd 등의 성과를 발표했으며, 지역 고품질 데이터의 공백을 채우기 위한 커뮤니티 데이터 대상의 Project Aquarium 플랫폼을 예고했습니다.`,
        aiRelevance: `峰会主线是东南亚低资源语言的大模型问题。主流 LLM 对英语等高资源语言表现好，对印尼语、菲律宾语、泰语、越南语等覆盖薄弱；SEA-LION 系列正是为填补这一空白而建。峰会围绕高质量区域数据的获取、版权与可及性等瓶颈展开——这些是低资源语言 LLM 能否做好的关键。`,
        aiRelevanceEn: `The summit's main theme is large language models for low-resource languages in Southeast Asia. Mainstream LLMs perform well on high-resource languages such as English but have weak coverage for Indonesian, Tagalog, Thai, Vietnamese, and other languages; the SEA-LION series was built precisely to fill this gap. The summit centered on bottlenecks including the acquisition of high-quality regional data, copyright, and accessibility—these are critical to whether low-resource language LLMs can perform well.`,
        aiRelevanceJa: `サミットの主線は、東南アジアの低リソース言語における大規模言語モデルの問題です。主流の LLM は英語などの高リソース言語に対して性能が良いですが、インドネシア語、フィリピン語、タイ語、ベトナム語などのカバレッジは弱いです。SEA-LION シリーズは正にこの空白を埋めるために構築されました。サミットは高品質地域データの取得、著作権と可及性などのボトルネックを中心に展開されました——これらは低リソース言語 LLM がうまくいくかどうかの鍵となるものです。`,
        aiRelevanceKo: `정상 회담의 주요 주제는 동남아 저자원 언어의 대규모 언어 모델 문제입니다. 주류 LLM은 영어 등 고자원 언어의 성능은 우수하지만, 인도네시아어, 필리핀어, 태국어, 베트남어 등의 커버리지는 부족합니다. SEA-LION 시리즈는 정확히 이 공백을 채우기 위해 개발되었습니다. 정상 회담은 고품질 지역 데이터의 획득, 저작권 및 접근성 등 병목 현상을 중심으로 진행되었습니다. 이들은 저자원 언어 LLM이 잘 작동할 수 있는지 여부의 핵심 요소입니다.`,
        singaporeRelevance: `SEA-LION 是 AISG 的旗舰开源模型，峰会体现新加坡把自己定位为东南亚 AI 枢纽——不只服务本国，而是牵头区域语言基础设施。第三届首次在境外（曼谷）举办、联合 Google 与泰国 VISTEC，是这种区域协作定位的直接信号。`,
        singaporeRelevanceEn: `SEA-LION is AISG's flagship open-source model, and the summit demonstrates Singapore's positioning of itself as Southeast Asia's AI hub—not just serving its own country, but leading regional language infrastructure. The third edition being held for the first time outside Singapore (in Bangkok) and jointly with Google and Thailand's VISTEC is a direct signal of this regional collaborative positioning.`,
        singaporeRelevanceJa: `SEA-LION は AISG のフラッグシップオープンソースモデルです。サミットはシンガポール自身を東南アジア AI ハブとして位置付けることを体現しています——国内サービスに限定されるのではなく、地域言語インフラの牽引を意味します。第 3 回が初めて国外（バンコク）で開催され、Google およびタイの VISTEC と共同で行われることは、このような地域協力ポジショニングの直接的な信号です。`,
        singaporeRelevanceKo: `SEA-LION은 AISG의 주력 오픈소스 모델이며, 정상 회담은 싱가포르가 자신을 동남아 AI 허브로 위치시킨다는 것을 보여줍니다. 자국만 서비스하는 것이 아니라 지역 언어 기반시설을 주도합니다. 제3회가 처음으로 해외(방콕)에서 개최되었으며 Google 및 태국 VISTEC과 협력한 것은 이러한 지역 협력 위치 결정의 직접적인 신호입니다.`,
        entityType: 'program',
        status: 'active',
        sources: [
          {
