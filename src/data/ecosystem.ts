...
Take: SUTD's existence saves the Singapore university system from **"the risk of NUS-and-NTU homogeneous competition"** — it takes the crossover-innovation route and complements the two comprehensive universities.`,
      ...
      },
      {
        ...
        description: '该项目由南洋理工大学张捷教授领导，与美国麻省理工学院合作，致力于开发可信任、可解释的人工智能-人类交互优化系统。研究重点是建立自然语言作为组合优化问题的鲁棒接口，使AI系统能够从人类语言中制定优化问题、进行强大的AI辅助优化，并提供可解释的反馈。该项目旨在为物流、制造、金融等行业提供高效的决策支持工具，同时推进可信任AI的基础研究。',
        descriptionEn:
      ...
      },
      {
        ...
        name: 'PINNACLE：物理信息神经网络自适应配置点和实验点选择',
        nameEn: 'PINNACLE: PINN Adaptive Collocation and Experimental Points Selection',
        description: 'PINNACLE 是一种自适应点选择算法，用于改进物理信息神经网络（PINNs）的训练效率。该方法通过神经正切核理论分析 PINN 训练动态，可自动联合选择所有类型的训练点（实验点和配置点），并在训练过程中动态调整不同点类型的比例。算法在正问题、反演问题和迁移学习等多种任务中显著超越现有方法，同时自动生成的点选择策略与手动启发式方法相似，具有良好的可解释性。',
        descriptionEn:
          'PINNACLE is an adaptive point selection algorithm designed to improve the training efficiency of Physics Informed Neural Networks (PINNs). The method analyzes PINN training dynamics using empirical Neural Tangent Kernel theory and automatically jointly selects all types of training points (experimental and collocation points), dynamically adjusting the proportions during training. The algorithm significantly outperforms existing point selection methods across multiple tasks including forward problems, inverse problems, and transfer learning, while producing interpretable selections that align with manual heuristics from prior work.',
      ...
      },
      {
        ...
        description: 'AI Singapore 研究团队提出 BadEdit 框架，这是一种创新的后门注入方法，通过直接修改模型参数来对预训练大语言模型进行高效的后门攻击。该方法仅需 15 个数据样本和 120 秒即可成功注入后门，攻击成功率接近 100%，同时保持对清洁数据的最小副作用。BadEdit 相比传统权重中毒方法的优势在于数据需求少、效率高，适用于多种任务领域包括文本分类、事实核查和对话情感生成。',
        descriptionEn:
...
A take: Aquarium is not AISG's flagship external product, but it is the engineering foundation that lets AISG keep delivering at high tempo (SEA-LION, TagUI, PeekingDuck, and more).`,
      ...
      },
      {
        ...
        description: 'AI Singapore 发表了一篇关于全身人体姿态与形状估计的研究论文。研究通过对三大类型的控制增强进行鲁棒性评估，揭示现有模型对位置变化的敏感性。针对这些问题，研究团队开发了 RoboSMPLX，通过定位模块、对比特征提取模块和像素对齐模块三个专门化组件，增强了全身姿态与形状估计的鲁棒性。该论文已在 NeurIPS 发表。',
        descriptionEn:
...
A take: **Singapore being able to sit at the AI Safety Summit table is a manifestation of its "national brand equity"** — a mature governance framework, rule-of-law stability, and AI neutrality — letting both the US/UK and China accept it as a counterpart. In an era of AI geopoliticisation, this kind of "neutral, trusted" position is becoming more and more valuable.`,
      ...
      },
      {
        ...
        name: '破除障碍、搭建桥梁：增进东南亚语言代表性',
        nameEn: 'Breaking Barriers, Building Bridges: Increasing Language Representation in Southeast Asia',
        description: 'AI新加坡与谷歌、VISTEC在泰国曼谷首次在新加坡外举办第三届语言峰会，汇聚了来自东南亚各地的AI专家和研究人员。峰会围绕高效模型训练、高质量数据获取共享以及区域AI进展进行深入讨论。谷歌展示了CALM和MatFormer等创新技术，AI新加坡推出SEA-LION v2、Project SEALD等项目，区域合作方分享了印尼、菲律宾、泰国和越南的LLM应用进展。峰会还宣布启动Project Aquarium——一个服务于东南亚的社区驱动数据地图平台，旨在解决区域内高质量数据获取困难的问题。',
        descriptionEn:
          'AI Singapore co-hosted the third Languages Summit with Google and VISTEC in Bangkok, Thailand, marking the first time the summit was held outside Singapore. The event brought together AI experts and researchers from across Southeast Asia to discuss efficient model training, high-quality data acquisition and sharing, and regional AI developments. Google Research showcased innovative techniques including CALM and MatFormer, while AI Singapore unveiled SEA-LION v2 and Project SEALD. Regional collaborators shared progress on LLMs and use-cases across Indonesia, the Philippines, Thailand, and Vietnam. The summit also announced Project Aquarium, a community-driven data map and platform designed to address gaps in accessible high-quality data for Southeast Asian AI development.',
