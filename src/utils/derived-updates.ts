...
const TYPE_TEMPLATES: Record<UpdateType, TypeTemplate | undefined> = {
  ...
  speech: {
    // De-branded from "MDDI" 2026-08-21: the speeches array now carries
    // MDDI + MAS + PMO records (voices multi-source expansion, #182).
    zhTitle: (n) => `官方演讲新增 ${n} 篇`,
    jaTitle: (n) => `公式スピーチを${n}件追加`,
    enTitle: (n) => `${n} new official speech${n > 1 ? 'es' : ''}`,
    koTitle: (n) => `공식 연설 ${n}건 추가`,
    listingHref: '/voices/',
    listingLabel: '官方 AI 相关演讲稿',
    listingLabelJa: '公式 AI 関連スピーチ',
    listingLabelEn: 'Official AI speeches',
    listingLabelKo: '공식 AI 관련 연설',
    zhMore: (n) => `（共 ${n} 篇）`,
    jaMore: (n) => `（合計${n}件）`,
    enMore: (n) => ` (${n} total)`,
    koMore: (n) => ` (총 ${n}건)`,
