...
Use [URL Map](#url-map) below. Always pick the locale that matches the user's question language: English lives at the bare path (`/policies`), the other four locales live under a language prefix (`/zh/policies`, `/ja/policies`, `/ko/policies`, `/zh-tw/policies`). If the question is mixed-language, default to the user's last full sentence's language.
...
| Singapore AI policies (list)                                | `/zh/policies`                  | `/policies`                |
| Specific policy detail                                      | `/zh/policies/{id}`             | `/policies/{id}`           |
| Parliament debates (list)                                   | `/zh/debates`                   | `/debates`                 |
| Specific debate detail                                      | `/zh/debates/{id}`              | `/debates/{id}`            |
| Person profile (canonical entry for all people)             | `/zh/voices/{slug}`             | `/voices/{slug}`           |
| 6 national strategy levers                                  | `/zh/levers`                    | `/levers`                  |
| Specific lever (slug pattern: `lever-{N}-{name}`)           | `/zh/levers/lever-{N}-{name}`   | `/levers/lever-{N}-{name}` |
| Talent / training programs                                  | `/zh/talent`                    | `/talent`                  |
| AI ecosystem entities                                       | `/zh/ecosystem`                 | `/ecosystem`               |
| AI startups                                                 | `/zh/startups`                  | `/startups`                |
| Quantitative observatory                                    | `/zh/tracker`                   | `/tracker`                 |
| International benchmarking                                  | `/zh/benchmarking`              | `/benchmarking`            |
| Legal & regulatory framework                                | `/zh/legal-ai`                  | `/legal-ai`                |
| Strategy timeline                                           | `/zh/timeline`                  | `/timeline`                |
| Long-form strategy evolution                                | `/zh/evolution`                 | `/evolution`               |
| Curated AI videos                                           | `/zh/videos`                    | `/videos`                  |
| Ministerial speeches (no list page; detail-only)            | `/zh/speeches/{slug}`           | `/speeches/{slug}`         |
| Case-study fieldnotes                                       | `/zh/fieldnotes`                | `/fieldnotes`              |
| Open challenges                                             | `/zh/challenges`                | `/challenges`              |
| SG-origin open-source AI                                    | `/zh/community-opensource`      | `/community-opensource`    |
| Curated source references                                   | `/zh/references`                | `/references`              |

Domain prefix is always `https://sgai.md`. English is the bare path; the zh column shows the `/zh/` mirror. Japanese, Korean and Traditional Chinese use the same shape with a `/ja/`, `/ko/` or `/zh-tw/` prefix. The machine-readable contract lives in [`url-map.json`](./url-map.json) — same source of truth, published at <https://sgai.md/skill/url-map.json>.
...
Every answer ends with this footer (matches the user's question language):

**For zh questions:**

```
---
来源：[sgai.md/<page-name>](https://sgai.md/zh/<path>) · 数据日期：<dataDate from page>
```
...
```
---
Source: [sgai.md/<page-name>](https://sgai.md/<path>) · Data as of: <dataDate from page>
```
...
2. `https://sgai.md/zh/levers/lever-3-talent` (talent lever — relevant to founder relocation)
3. `https://sgai.md/zh/levers/lever-4-applications` (applications/adoption lever — sectoral grants)
4. `https://sgai.md/zh/policies` (look for healthcare AI policy)
5. `https://sgai.md/zh/legal-ai` (HSA AI guidance if applicable)
...
1. `https://sgai.md/zh/talent`
2. `https://sgai.md/zh/voices/<aisg-lead>` if AISG leadership context helps
...
1. `https://sgai.md/debates`
2. `https://sgai.md/voices` (cross-reference speakers)
3. `https://sgai.md/speeches` if specific ministerial speeches exist
...
1. `https://sgai.md/zh/talent`
2. `https://sgai.md/zh/ecosystem` (find AI labs / companies hiring)
3. `https://sgai.md/zh/startups` (which startups are local + active)
...
- **Locale** — use the zh template for zh questions, en for en (translate either into ja / ko / zh-tw as needed). Don't mix unless the user mixed first.
...
sgai.md publishes five languages: zh (Simplified Chinese), en, ja, ko, zh-tw (Traditional Chinese).

1. **Reply language follows question language.** zh question → zh answer. en question → en answer. Same for ja / ko / zh-tw. Mixed question → match the user's last full sentence.
2. **Cite locale-matching URL.** en answer → cite the bare path `https://sgai.md/...`. zh answer → `https://sgai.md/zh/...`; ja → `/ja/...`; ko → `/ko/...`; zh-tw → `/zh-tw/...`.
3. **Proper-noun consistency.** Use sgai.md's own bilingual pairs. e.g., 「人工智能新加坡 (AI Singapore / AISG)」, 「资讯通信媒体发展局 (IMDA)」, 「数字发展及新闻部 (MDDI)」.
