...
async function main() {
  ...
  // Surface to homepage updates feed.
  const UPDATES_TS = resolve(ROOT, 'src/data/updates.ts');
  if (approved.length > 0) {
    try {
      ...
      console.warn(`  ⚠ updates feed append failed: ${err instanceof Error ? err.message : err}`);
    }
  }
...
${approved.map((e) => `- ${e.id} [${e.date}] ${e.fields.title}`).join('\n')}`;
  ...
  if (approved.length > 0) commitFiles.push(UPDATES_TS);
