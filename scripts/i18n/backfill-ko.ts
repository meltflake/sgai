// scripts/i18n/backfill-ko.ts
// ────────────────────────────────────────────────────────────────────────
// Phase 2 backfill: walk a sgai data .ts file, find every zh field that's
// missing a `${field}Ko` sibling, translate the zh value via Claude haiku
// (zh→ko), and splice the new `${field}Ko: '…'` line into the source —
...
//
// USAGE
//   npx tsx scripts/i18n/backfill-ko.ts --file=src/data/policies.ts
//   npx tsx scripts/i18n/backfill-ko.ts --file=src/data/policies.ts --dry-run
//   npx tsx scripts/i18n/backfill-ko.ts --file=src/data/policies.ts --limit=10
//   npx tsx scripts/i18n/backfill-ko.ts --all   # all 13 data files
//
// Caveats:
//   - Inserts the new line immediately after the zh field's VALUE (which
//     may span multiple lines for Prettier-wrapped strings).
//   - Skips fields whose zh value is empty or already has *Ko set.
...
const ZH_TO_KO_SYSTEM_PROMPT = [
  ...
  '1. If the SOURCE text contains quotation marks (ASCII " or Chinese " "), CONVERT them in the output to Korean 「 」 (or 『 』 for nested) or to curly typographic quotes (" and ").',
  ...
  '3. The translated paragraph must NEVER contain ASCII straight quotes (") — they would break JSON parsing. Use 「 」, curly quotes, or no quotes at all.',
  '',
  ...
  while (i < argv.length) {
    ...
    if (a === '--all') {
      ...
      process.stdout.write(
        'Usage: backfill-ko.ts --file=<path> [--file=<path>...] [--limit=N] [--dry-run] [--force]\n' +
          '       backfill-ko.ts --all [--limit=N] [--dry-run] [--force]\n' +
          '\n' +
...
async function backfillFile(filePath: string, opts: { dryRun: boolean; limit?: number; force?: boolean }): Promise<{
  ...
  process.stderr.write(`\n[backfill-ko] ${filePath}\n`);

  // Only check 'ko' here — Phase 0/1 enforced 'en'; Phase 2 enforced 'ja'. This phase fills 'ko'.
  const allIssues = findUnpairedFields(abs, { fields: TARGET_FIELDS, locales: ['ko'] });
  // Only handle 'missing-sibling'. 'empty-sibling' (where *Ko: '' is
  ...
  process.stderr.write(`  found ${issues.length} unpaired field(s) needing *Ko\n`);
  if (issues.length === 0) {
  ...
  //      inside a multi-line object. Insert the new `fieldKo: '<ja>',`
  ...
  for (const [lineIdx, arr] of byLine.entries()) {
    ...
    for (const ins of arr) {
      ...
      const inject = ` ${ins.field}Ko: '${escaped}',`;
  ...
  for (const ins of newLineIns) {
    ...
    const newLine = `${ins.indent}${ins.field}Ko: ${ins.quote}${escaped}${ins.quote},`;
  ...
  process.stderr.write(`  wrote ${insertions.length} new *Ko line(s)\n`);
...
async function main() {
  ...
  process.stderr.write(
    `\n[backfill-ko] DONE — files=${args.files.length} found=${totals.found} translated=${totals.translated} inserted=${totals.inserted} skipped=${totals.skipped}\n`
  );
  if (args.dryRun) {
    process.stderr.write(`[backfill-ko] (--dry-run: no files modified)\n`);
  } else if (totals.inserted > 0) {
    process.stderr.write(`[backfill-ko] Run \`npm run fix:prettier\` to normalize formatting.\n`);
  }
}
...
if (isEntryPoint()) {
  main().catch((err) => {
    process.stderr.write(`[backfill-ko] ERROR: ${err instanceof Error ? err.stack || err.message : String(err)}\n`);
    process.exit(1);
  });
}
