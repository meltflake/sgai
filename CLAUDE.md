...
# = check:i18n + check:schema + check:meta + check:zh-tw-misconversion + check:data-export
```
...
| `check:data-export` | `scripts/evals/data-export/check.ts` | `dist/data/*.json` 的信封契约：能 parse；`schemaVersion === 1`；`dataset` 等于文件名；`count === items.length`；`license.terms` 指向 DATA-LICENSE.md；凡带 `links.sgai` 的行必须五语齐全、每条都是绝对 `https://sgai.md/` 地址且前缀与 key 一致（en 无前缀）；`records.json` 每行 `title.en` 非空。                                                                                  | 动了 `src/pages/data/*.ts` / [src/utils/data-export.ts](src/utils/data-export.ts) |
