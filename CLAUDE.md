...
# = check:i18n + check:schema + check:meta + check:zh-tw-misconversion + check:markdown-export
```
...
| `check:markdown-export` | `scripts/evals/markdown-export/check.ts` | `dist/**/*.md` 的详情页 Markdown 孪生（排除 `dist/skill/`）：每个板块抽样至多 20 个文件，断言首行是 `# ` H1、含 `- sgai: https://sgai.md/` 永久链接行、含 `CC BY 4.0` 许可标记、无 `undefined` / `[object Object]` 残留。`check:i18n` 只扫 `*.html`，孪生文件在它的盲区里。                                                                                                          | 动了 `src/utils/markdown-export.ts`、`[id].md.ts` 路由、`src/utils/license.ts` |
