...
| `03_enrich_debates.py`   | AI 生成中文摘要              | OpenAI API key | **需 API key**，Codex 可直接完成此步 |
| `04_analyze_patterns.py` | AI 分析政策模式              | OpenAI API key | **需 API key**，Codex 可直接完成此步 |
...
**实际操作方式**：步骤 1-2 可通过直接调用 SPRS API 替代脚本，步骤 3-4 由 Codex 直接完成（无需 OpenAI API），步骤 5 可手动编辑 `debates.ts`。即：可以完全跳过 Python 脚本，直接用 API + Codex 完成全流程。
